...
- 스프링 프로젝트 모음(https://spring.io/projects)
  - ** 스프링 프레임워크 - 핵심기술
  - 스프링 부트 - 기술들을 편리하게 사용
    - 최근에 기본으로 사용
    - Tomcat 같은 웹 서버를 내장해서 별도의 웹 서버 설치 필요없음
    - starter 종속성 지원 : 빌드 구성 쉽게
    - 3rd party 라이브러리 자동 구성 (호환되는 버전 자동 선택)
    - production ready 기능(메트릭, 상태확인, 외부구성)
    - 설정보다 관례(CoC; Convention over Configuration) 간결한 설정


## 객체 지향 프로그래밍

- **객체들**의 **모임**, 각각 **객체**는 **메시지**를 주고받고, 데이터를 처리할 수 있다.**(협력)**
- 프로그램을 **유연**하고 **변경**이 용이하게 만듦.

### 유연하고, 변경이 용이!

- 궁극의 유연함, 변경이 가능한 방법? => 다형성
...
- 클라이언트의 영향을 주지 않고, 새로운 기능을 제공할 수 있다.
...
- 객체 설계시 역할(인터페이스)를 먼저 부여하고, 그 여할을 수행하는 구현 객체 만들기
...
- (인터페이스 뿐만이 아니라) 클래스 상속 관계도 다형성, 오버라이딩 가능함.
  ![객체는 협력하는 관계](./image/java-overriding.png)
...
- 스프링은 구현을 편리하게 변경할 수 있도록 돕는 역할! (마치 레고 블럭 조립하듯이)
...
- 하지만 모호함이 있다.
  - 문맥과 상황에 따라 다름
- 기준은 **변경**에 있다. 변경이 적게 분리한 것이면 잘 따른것
  - 예) UI변경, 객체의 생성과 사용을 분리

### [중요!] OCP: 개방-폐쇠 원칙 (Open/closed principle)

- 확장에는 열려 있으나, 변경에는 닫혀 있어야 한다.
- **다형성**을 활용하라는 말
- 인터페이스를 구현한 새로운 클래스를 하나 만들어서 새로운 기능 구현
- 역할과 구현의 분리를 생각해보자!
- 객체를 생성하고, 연관관계를 맺어주는 별도의 조림, 설정자가 필요하다.(스프링 DI 컨테이너)

### LSP: 리스코프 치환 원칙 (Liskov substitution principle)

- 다형성에서 하위 클래스는 인터페이스 규약을 다 지켜야 한다는 것, 인터페이스를 구현한 구현체를 믿고 사용하려면 이 원칙이 필요
...
- 분리하면 무엇이 좋은가?
  - 정비 인터페이스 자체가 변해도 운전자 클라이어트에 영향 주지 않음
  - 인터페이스가 명확해짐, 대체 가능성 높아짐

### [중요!] DIP: 의존 관계 역전의 원칙 (Dependency inversion principle)

- 추상화에 의존해야지, 구체에 의존하면 안된다.
- 구현 클래스에 의존말고, 인터페이스에 의존하라는 뜻
- 앞서 이야기한 **역할에 의존**해야지 구현에 의존하면 안되는 것과 동일한 이야기이다.

### 정리

- 객체지향 핵심은 다형성
- 다형성 만으로는 구현 객체를 변경할 때 클라이언트 코드도 함께 변경됨
- **_다형성 만으로 DIP, OCP 를 지킬 수 없음_**
- one more thing -> 뭘까?
