...
- 다른점: 실행하는 시점에 원하는 전략(알고리즘) 을 전달(선택) 가능하다.
- 실행 그림

![전략패턴 - 파라메터 전달 사용](img/strategy-params.png)

- 장점
  - 실행할 때마다 전략을 유연하게 변경 가능
- 단점
  - 실행할 때마다 전략을 계속 지정해 주어야 함


- 우리가 해결하고 싶은 문제
  - 변하는 부분(알고리즘) / 변하지 않는 부분(탬플릿) 분리
...
- 콜백의 정의
  - `callback` 은 코드가 호출(`call`)은 되는데 코드를 넘겨준 곳의 뒤(`back`)에서 실행된다.
...
- 자바에서의 콜백
  - 실행가능한 코드를 인수로 넘기려면 객체가 필요함(일급시민 아니므로)
  - java8 이전: 익명 클래스
  - java8 이후: 람다 식
- 스프링에서의 콜백
